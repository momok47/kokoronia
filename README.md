...
### OpenAPIからSDKを生成
OpenAPIスキーマは `drf-spectacular` で動的生成しています。
...
### 共通HTTPクライアント
`src/core/clients` にタイムアウト・リトライ・サーキットブレーカを備えた共通クライアントと、API専用クライアントを用意しています。

- `HttpClient` : タイムアウト/リトライ/サーキットブレーカ対応
- `ConversationApiClient` : `/api/v1/topics/*` 呼び出し
- `Idempotency-Key` と `X-Request-Id` を自動付与
