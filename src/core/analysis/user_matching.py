...
def test_database_matching():
    ...
    result = matcher.run_database_matching(
        ...
        save_results=True          # 結果保存（現在は表示のみ）
    )
    
    if result['matches']:
        print(f"\n=== 最終結果 ===")
        print(f"マッチング成功: {len(result['matches'])}組")
        print(f"総合満足度: {result['total_compatibility']:.4f}")
        print(f"平均相性: {result['average_compatibility']:.4f}")
    else:
        print("マッチング対象のユーザーが見つかりませんでした。")

def test_mock_matching():
    """モックデータマッチングのテスト"""
    print("=== モックデータマッチングテスト ===")
    
    # モックデータ作成
    mock_df = create_mock_data(n_users=8)  # 8人でテスト
    
    # マッチング実行
    matcher = UserMatcher()
    active_users = matcher.filter_active_users(mock_df)
    
    print(f"\n=== ユーザー情報 ===")
    for _, user in active_users.iterrows():
        print(f"{user['user_id']}: トップトピック - {user[matcher.topic_labels].idxmax()} ({user[matcher.topic_labels].max():.4f})")
    
    result = matcher.run_matching(active_users)
...
if __name__ == "__main__":
    ...
    if DJANGO_AVAILABLE:
        try:
            ...
            print(f"データベーステストでエラー: {e}")
            print("モックデータテストに切り替えます...\n")
            test_mock_matching()
    else:
        print("Django環境が利用できません。モックデータテストを実行します...\n")
        test_mock_matching()
